...
public class MemberApiController {
    ...
    public UpdateMemberResponse updateMemberV2(@PathVariable("id") Long id, @RequestBody @Valid UpdateMemberRequest request) {
        memberService.updateMember(id, request.getName()); // update logic
        Member findMember = memberService.findOne(id); // seperate query logic to update logic
        return new UpdateMemberResponse(findMember.getId(), findMember.getName());
    }

    @GetMapping("/api/v1/members")
    public List<Member> getMembersV1() {
        return memberService.findMembers(); // do not directly expose entity @ production service
    }
